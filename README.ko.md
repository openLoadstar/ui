...
처리 과정을 mermaid 그림 한 장으로 기술하는 요소입니다. `### DIAGRAM` 안은 **순수 mermaid**라 마크다운이 렌더되는 곳이면 어디서든(GitHub 포함) 그대로 그려지고, 각 노드가 어떤 WP / DWP / 다른 FLOW를 가리키는지는 코드블록 **밖**의 `### REFERENCES`에 적습니다 — 그래야 구조 추출기와 검증기가 그 관계를 볼 수 있습니다.

도형에 의미가 고정돼 있습니다: `[단계]`, `{분기}`, `(( ))` 병합, `[[하위 흐름]]`, `[(저장소)]`, `((시작·끝))`.
...
|:--|:--|
| 흐름 만들기 | 툴바 **+ FLOW** |
| 모드 전환 | FLOW 탭의 `👁 보기` · `✎ 그림 편집` · `</> 텍스트 편집` |
| 노드 고르기 | 그림에서 클릭 — 우측 패널에 id·종류·라벨이 뜹니다 |
| 단계 늘리기 | **⑂ 병렬 추가**(갈래를 하나 더) · **↳ 삽입 추가**(기존 갈래를 밀고 사이에 끼움) |
| 한 갈래에만 끼우기 | 화살표 목록의 **↳** — 조건 라벨은 분기 쪽에 남습니다 |
| 조건 이름 붙이기 | 화살표 목록의 입력칸 |
| 갈래 합치기 | 화살표의 **상대 노드 드롭다운**을 바꾸거나, **연결**로 새 화살표 긋기 |
| 노드 옮기기 | **이동** — 고른 노드를 다른 노드 뒤로 |
| 요소 연결 | 패널 아래 목록에서 WP/DWP/FLOW 고르기 (FLOW를 고르면 도형이 하위 흐름으로 바뀝니다) |
| 하위 흐름 만들기 | **Ctrl+클릭**으로 여러 개 고른 뒤 **⊞** — 같은 그림 안의 `subgraph` 구역이 됩니다 |
| 접기·펼치기 | 보기 모드 문서 위의 **칩**(`⊖` 펼침 / `⊕` 접힘). 파일은 바뀌지 않습니다 |
| 구역 이름·해제 | 구역 안 노드를 고르면 제목 입력칸과 **⊟**(해제) |
| 배율 | 같은 막대의 `−` / `+` / `맞춤`, 또는 `Ctrl`+휠. `맞춤`은 창 폭에 맞추고, 옆의 퍼센트가 그때의 실제 배율입니다 |
| 되돌리기 | `Ctrl+Z` |

보기 모드에서는 요소가 연결된 노드에 테두리가 생기고, 누르면 그 요소가 탭으로 열립니다.

mermaid는 그림을 지면 폭에 맞춰 줄이기 때문에 노드가 수십 개가 되면 글자가 읽기 어려워집니다. 확대하면 그 제한을 풀고 뷰어가 대신 스크롤합니다. 배율은 탭이 기억하므로 구역을 접거나 그림을 편집해도 유지됩니다.

모든 편집은 고쳐야 할 **줄만** 고칩니다 — 주석이나 손으로 쓴 표기는 그대로 남습니다. 편집기가 안전하게 다룰 수 없는 문법(한 줄 체인 `a --> b --> c`, 중첩 `subgraph` 등)이 섞인 그림은 망가뜨리는 대신 구조 편집만 잠그고, 그리기와 요소 연결은 계속 됩니다. 자세한 규약은 [appendix/FLOW.md](https://github.com/openLoadstar/spec/blob/main/SPEC%202.0/appendix/FLOW.md).
