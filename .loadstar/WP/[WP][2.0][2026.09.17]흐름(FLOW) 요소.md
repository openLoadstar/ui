...
- 삭제 시 **지워지는 줄에 이웃 노드의 정의가 실려 있으면 그 도형·라벨이 날아가는** 버그가 있었다(`n --> parse[[요소 파싱]]`에서 n을 지우면 parse가 맨 사각형이 됨). 지우기 전에 정의를 모아뒀다가 남은 줄에 되돌려 놓는 방식으로 고쳤다(`collectDefinitions`/`restoreDefinitions`). 줄 단위 수술이 흔히 빠지는 함정이라 비슷한 편집을 추가할 때 같이 확인해야 한다.
