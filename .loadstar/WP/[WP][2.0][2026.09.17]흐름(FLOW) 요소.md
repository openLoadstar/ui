...
- [x] 2026-09-17 (E3) "이 노드를 새 FLOW로 빼내기" — 새 FLOW 파일 생성(SUMMARY=노드 라벨, PARENT=현재 흐름, 첫 단계를 그 라벨로 씨앗), 원래 자리는 `[[ ]]` 하위 흐름 도형으로 바꾸고 `REFERENCES`가 새 파일을 가리키게 한 뒤 새 탭으로 연다(`TabManager.extractToFlow`)
