...
- 묶을 수 있는 조건: **드나드는 화살표가 각각 최대 하나**여야 한다. 부모에 `[[ ]]` 한 칸으로 줄여야 하고, 자식의 입구·출구가 하나로 정해져야 펼침도 되기 때문이다(`groupBoundary`). 여러 갈래면 사유를 패널에 띄우고 묶지 않는다.
- 풀기는 **자식 파일을 지우지 않는다** — 연결만 끊는다. 되돌릴 수 없는 삭제를 조용히 하지 않기 위해서고, 남은 파일은 트리에서 보이므로 사용자가 판단하면 된다.
- **하위 흐름을 새로 만드는 통로**: 이제 묶기가 그 역할을 한다(노드 하나만 골라도 묶인다). 앞서 지운 "빼내기"는 이 기능의 노드 한 개짜리 특수 경우였다.
...
### COMMENT
**하위 흐름을 "그룹"으로 다루기로 한 이유** (2026-09-17, 사용자 결정): 하위 흐름은 결국 노드 몇 개를 한 칸으로 접는 일이라 그룹에 가깝다. 그래서 편집도 "자식을 펼친 채로 고치기"(탭 하나가 두 파일을 수정하는 상태가 되어 저장·되돌리기·git 이력이 갈라진다)가 아니라 **경계를 다루는 묶기·풀기**로 갔다. 풀기를 짝으로 둔 것이 핵심이다 — 되돌릴 수 없는 구조 변경은 쓰기 무섭다.
